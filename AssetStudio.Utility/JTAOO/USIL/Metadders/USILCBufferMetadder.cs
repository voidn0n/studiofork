	...
	public class USILCBufferMetadder : IUSILOptimizer
	{
		...
		private void UseMetadata(USILOperand operand)
		{
			if (operand.operandType == USILOperandType.ConstantBuffer)
			{
				...
				HashSet<NumericShaderParameterJ> cbParams = new HashSet<NumericShaderParameterJ>();
				List<int> cbMasks = new List<int>();

				BufferBindingJ binding = _shaderData.ConstantBufferBindings.First(b => b.Index == cbRegIdx);
				ConstantBufferJ constantBuffer = _shaderData.ConstantBuffers.First(b => b.Name == binding.Name); 

                // Search children fields
                foreach (NumericShaderParameterJ param in constantBuffer.AllNumericParams)
				{
				...
				foreach (StructParameterJ stParam in constantBuffer.StructParams)
				{
					foreach (NumericShaderParameterJ cbParam in stParam.AllNumericMembers)
					{
				...
				if (cbParams.Count > 1)
				{
					...
					foreach (NumericShaderParameterJ param in cbParams)
					{
				...
				{
					NumericShaderParameterJ param = cbParams.First();
