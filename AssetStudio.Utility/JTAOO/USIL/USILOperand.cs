		...
		public string ToString(bool forceHideMask)
		{
			...
			else
			{
				switch (operandType)
				{
					...
					default:
						{
							if (DXShaderNamingUtilsJ.HasSpecialInputOutputName(operandType))
							{
								body = DXShaderNamingUtilsJ.GetSpecialInputOutputName(operandType);
							}
							break;
						}
				};
			...
			if (operandType != USILOperandType.ImmediateFloat &&
				...
				!DXShaderNamingUtilsJ.HasSpecialInputOutputName(operandType) &&
				displayMaskOverride)
			{
				if (mask.Length > 0)
				{
