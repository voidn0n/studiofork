...
namespace ShaderLabConvert
{
	...
	{
		private DirectXCompiledShaderJ _dxShader;
		...
		public DirectXProgramToUSIL(DirectXCompiledShaderJ dxShader)
		{
			_dxShader = dxShader;
		...
		private void ConvertInputs()
		{
			...
			for (int i = 0; i < dxInputs.Length; i++)
			{
				...
				string inputName = DXShaderNamingUtilsJ.GetISGNInputName(dxInput);
		...
		private void ConvertOutputs()
		{
			...
			for (int i = 0; i < dxOutputs.Length; i++)
			{
				...
				string outputName = DXShaderNamingUtilsJ.GetOSGNOutputName(dxOutput);
		...
		private void AddOutputLocal()
		{
			...
			{
				AssetStudioUtility.Type.Vertex => USILConstants.VERT_OUTPUT_LOCAL_NAME,
                AssetStudioUtility.Type.Pixel => USILConstants.FRAG_OUTPUT_LOCAL_NAME,
				_ => "o" // ?
			};
			string outputStructName = _dxShader.Shdr.shaderType switch
			{
                AssetStudioUtility.Type.Vertex => USILConstants.VERT_TO_FRAG_STRUCT_NAME,
                AssetStudioUtility.Type.Pixel => USILConstants.FRAG_OUTPUT_STRUCT_NAME,
				_ => "o" // ?
			};
